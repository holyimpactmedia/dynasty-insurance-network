  ...
  const { error } = await resend.emails.send({
    ...
    html: `
      <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:32px;color:#111827">
        <h1 style="color:#0A2540;font-size:24px">${subject}</h1>
        <p style="line-height:1.6">Use the secure link below to ${action.toLowerCase()}.</p>
        <p><a href="${url}" style="display:inline-block;background:#0A2540;color:#ffffff;padding:12px 20px;border-radius:6px;text-decoration:none;font-weight:700">${action}</a></p>
